#this will take the excel file created by the filesfrom20XX ,
# and you will be able to delete files by mame and time, there is also a
# wild card feature, * , to delete all files within a specific 
#directory or time
...
function FindFileIndex {
    ...
    if ($searchPath) {
        for ($i = 0; $i -lt $data.Count; $i++) {
            if ($data[$i].Path -like $searchPath) {
                return $i
            }
        }
    }
...
function ManualSpecificMode {
    $manualInput = Read-Host "Enter a specific file path or last accessed time (yyyy-MM-dd HH:mm) or wildcard pattern"
    $wildcard = $null
    ...
    } elseif ($manualInput -match '^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$') {
    ...
    } elseif ($manualInput -match '^\S+\\\*$') {
        # If wildcard path is provided
        $wildcardPath = $manualInput
        $searchPath = $wildcardPath -replace '\\\*$', '\*'
        $index = FindFileIndex -data $data -searchPath $searchPath
        if ($index -ne -1) {
            for ($i = $index; $i -lt $data.Count; $i++) {
                $filePath = $data[$i].Path
                $lastAccessTime = $data[$i].LastAccessed
            ...
            Write-Host "Files not found for the specified path."
        }
    } else {
...
if (Test-Path -Path $excelFile) {
    ...
    if ($skipToFile -eq 'Y') {
        ...
        if ($searchType -eq 'Path') {
            $searchPath = Read-Host "Enter the file path to search for (e.g., P:\ARCHIVES\*\Vedder River Poultry\*)"
